...
export default function EditProfilePage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        set_bio((raw?.speaker as any)?.bio ?? "");

        const union_ids = new Set<number>();
        raw?.speaker?.languages?.forEach((l) => union_ids.add(Number(l.id)));
  ...
  async function handle_save() {
    try {
      ...
      const bio_norm = ensure_length(trim_or_none(bio), undefined, 320);
      if (bio_norm !== null) payload.bio = bio_norm;
      ...
      if (!keys.length) {
        ...
        router.push("/palestrante/perfil?updated=1");
        return;
      }
